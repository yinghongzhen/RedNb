...
using RedNb.Base.EntityFrameworkCore;
using RedNb.Base.MultiTenancy;
...
public class BaseHttpApiHostModule : AbpModule
{
    ...
    {
        Configure<AbpDistributedCacheOptions>(options => { options.KeyPrefix = "Base:"; });
    }
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<BaseDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}RedNb.Base.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<BaseDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}RedNb.Base.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<BaseApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}RedNb.Base.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<BaseApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}RedNb.Base.Application"));
            });
        }
    }
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("Base");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "Base-Protection-Keys");
        }
    }
