...
using RedNb.WebGateway.MultiTenancy;
...
[DependsOn(
    typeof(WebGatewayHttpApiModule),
...
public class WebGatewayHttpApiHostModule : AbpModule
{
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("WebGateway");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "WebGateway-Protection-Keys");
        }
    }
